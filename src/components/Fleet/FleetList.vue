<template>
  ...
  <v-container fluid class="px-8 py-2 flex-grow-1 overflow-y-auto" style="scrollbar-width: thin;">
        ...
        <v-card class="fleet-mini-card" density="compact" :variant="store.activeIcao === f.icao24 ? 'tonal' : 'flat'"
          ...
          <v-card-text class="pa-2">
            ...
            <div class="d-flex justify-space-between align-center mb-1">
              <span class="text-caption font-weight-bold" style="font-size: 11px;">{{ f.callsign || 'İHA' }}</span>
              <v-icon :color="f.energy < 20 ? 'error' : 'success'" size="8">mdi-circle</v-icon>
            </div>

            <!-- Hız (kt) ve İrtifa (ft) bilgilerini yan yana dizen alan. -->
            <div class="d-flex justify-space-between mb-2 text-caption" style="font-size: 10px; opacity: 0.8;">
              <span><v-icon icon="mdi-gauge" size="12" class="me-1" />{{ Math.round(f.velocity) }} kt</span>
              <span><v-icon icon="mdi-image-filter-hdr" size="12" class="me-1" />{{ Math.round(f.baroaltitude) }} ft</span>
            </div>
