  ...
  <v-container fluid class="px-3 py-2 flex-grow-1 overflow-y-auto" style="scrollbar-width: thin;">
    ...
    <v-row no-gutters>
      ...
      <v-col v-for="f in store.filteredFlights" :key="f.icao24" cols="6" class="pa-0" style="padding: 2px;">
        ...
        <v-card class="fleet-mini-card" :variant="store.activeIcao === f.icao24 ? 'tonal' : 'flat'"
          ...
          <v-card-text class="pa-3">
            ...
            <div class="d-flex justify-space-between align-center mb-2">
              <span class="text-caption font-weight-bold" style="font-size: 12px;">{{ f.callsign || 'İHA' }}</span>
            ...
            <div class="d-flex justify-space-between mb-3 text-caption" style="font-size: 10px; opacity: 0.8;">
              <span style="display: inline-flex; align-items: center;"><v-icon icon="mdi-gauge" size="12" class="me-1" />{{ Math.round(f.velocity) }} kt</span>
              <span style="display: inline-flex; align-items: center;"><v-icon icon="mdi-image-filter-hdr" size="12" class="me-1" />{{ Math.round(f.baroaltitude) }} ft</span>
            </div>
